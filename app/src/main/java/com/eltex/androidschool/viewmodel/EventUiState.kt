...
data class EventUiState(
    ...
    val emptyError: Throwable? = (status as? Status.Error)?.reason?.takeIf { events == null }
    val refreshingError: Throwable? = (status as? Status.Error)?.reason?.takeIf { events != null }
}